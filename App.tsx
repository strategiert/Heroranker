  ...
  return (
    <div className="flex flex-col h-screen bg-[#f0f4f8] text-slate-900 font-sans overflow-hidden select-none relative">
        <main className="flex-1 overflow-hidden relative">
            <OfflineToast />
            <AssistantUI />
            <MissionGuide onNavigate={(v) => setView(v as any)} />
                ...
                <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-slate-900 w-full max-w-4xl h-[80vh] rounded-3xl overflow-hidden flex shadow-2xl border border-slate-700">
                        ...
                        <div className="w-1/2 bg-slate-950 relative hidden md:block">
                            {selectedHero.image && typeof selectedHero.image !== 'string' && selectedHero.image.url ? (
                                <img src={selectedHero.image.url} className="w-full h-full object-cover opacity-80" />
                            ) : (
                            ...
                            <div className="absolute bottom-8 left-8">
                                ...
                                <p className="text-slate-400 text-xs font-bold uppercase mt-1">{selectedHero.appearance?.race} // {selectedHero.biography?.alignment}</p>
                            </div>
                        </div>
                        ...
                        <div className="w-full md:w-1/2 bg-slate-900 flex flex-col relative">
                            ...
                            <div className="h-48 md:hidden relative shrink-0">
                                {selectedHero.image && typeof selectedHero.image !== 'string' && selectedHero.image.url && (
                                    <img src={selectedHero.image.url} className="w-full h-full object-cover" />
                                )}
                            ...
                            <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="bg-slate-800 p-3 rounded-xl border border-slate-700">
                                        ...
                                        <div className="text-lg font-black text-white">{selectedHero.powerstats?.strength}</div>
                                    </div>
                                    <div className="bg-slate-800 p-3 rounded-xl border border-slate-700">
                                        <div className="text-xs text-slate-500 font-bold uppercase">Intelligenz</div>
                                        <div className="text-lg font-black text-white">{selectedHero.powerstats?.intelligence}</div>
                                    </div>
                                    <div className="bg-slate-800 p-3 rounded-xl border border-slate-700">
                                        <div className="text-xs text-slate-500 font-bold uppercase">Geschwindigkeit</div>
                                        <div className="text-lg font-black text-white">{selectedHero.powerstats?.speed}</div>
                                    </div>
                                    <div className="bg-slate-800 p-3 rounded-xl border border-slate-700">
                                        <div className="text-xs text-slate-500 font-bold uppercase">Power</div>
                                        <div className="text-lg font-black text-white">{selectedHero.powerstats?.power}</div>
                                    </div>
                                </div>
