...
interface SpireScreenInnerProps {
  unlockedHeroes: SpireHero[];
}

function SpireScreenInner({ unlockedHeroes }: SpireScreenInnerProps) {
  const {
    currentFloor,
    highScore,
    selectedHero,
  ...
  return (
    ...
    >
      ...
      {/* FIXED ACTION FOOTER */}
      <div className="shrink-0 bg-slate-900/95 border-t border-slate-800 p-3 pb-safe-bottom space-y-2 z-20">
        {!selectedHero ? (
          /* Hero Select Button */
          <button
...
export const SpireScreen = ({ myHeroes, onNavigateToRecruit }: any) => {
   // Transform App Heroes to Spire Heroes
   const spireHeroes = useMemo<SpireHero[]>(() => {
      return myHeroes.map((h: AppHero) => {
        ...
        return {
          id: h.id,
          name: h.name,
          image: h.image?.url ? <img src={h.image.url} className="w-full h-full object-cover rounded-lg" alt={h.name} /> : '🦸',
          faction,
   ...
   if (spireHeroes.length === 0) {
      return (
        <div className="h-full flex flex-col items-center justify-center p-8 text-center bg-[#020617] text-white">
            ...
            <h2 className="text-xl font-black mb-2 tracking-wide">SPIRE LOCKED</h2>
            <p className="text-slate-400 mb-8 text-sm max-w-xs mx-auto leading-relaxed">
              Du benötigst mindestens einen Champion, um den Turm zu betreten.
            </p>
            <button 
              onClick={onNavigateToRecruit} 
              className="px-8 py-4 bg-blue-600 hover:bg-blue-500 active:bg-blue-700 rounded-xl font-bold shadow-[0_4px_0_rgb(30,58,138)] active:shadow-none active:translate-y-[4px] transition-all uppercase tracking-widest text-sm"
            >
               Rekrutieren
            </button>
        </div>
   ...
   return (
        <SpireScreenInner unlockedHeroes={spireHeroes} />
   );
};
