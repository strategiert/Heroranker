...
export const BuildingTile: React.FC<BuildingTileProps> = ({ building, onSelect, onSpeedUp }) => {
  ...
  // Reset image state when skin or building type changes
  useEffect(() => {
      setImgError(false);
  ...
  // Check Upgrade Availability for Visual Cue
  ...
  // Show Green Arrow if: Not upgrading, Not max level, Can afford
  const showUpgradeArrow = !isUpgrading && !isMaxLevel && canAfford && building.level > 0;

  // Determine Resource Icon for Production Buildings (Visual "Ready to Collect" cues)
  ...
  // Simple "Ready to Collect" simulation (randomly true for visual feel if idle)
  // In a real game, this would check `building.lastCollectedTime`
  ...
  return (
    <div 
        ...
        className={`
            ...
            ${imgError ? skin.styleClass : ''} 
            ${!imgError ? 'hover:scale-105 duration-300' : ''}
        `}
    >
        {/* === FLOATING UI BUBBLES (The "Top War" Look) === */}
        ...
        {/* Assigned Hero Badge (Replaces Icon Badge) */}
