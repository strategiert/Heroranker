...
export const RecruitmentCenter: React.FC<RecruitmentCenterProps> = ({ importedHeroes, onHeroRecruited }) => {
  ...
  const handleSummon = async (type: 'standard' | 'elite') => {
    ...
    if (type === 'standard' && standardTicketCount > 0) {
       // Ideally removeItem checks existence, assume true for now or better implementation in InventoryContext
       // For MVP, we just decrement visually or rely on context. 
       // Since removeItem stub returns true, we assume it works.
       // Note: real implementation needs proper item removal by ID. 
       // Assuming InventoryContext has a way to remove by templateId or we find one.
       // We'll simulate removal for now as context might be basic.
       const ticket = inventory.consumables.find(i => i.templateId === 'recruit_ticket');
       if(ticket) {
           // Hacky removal for MVP if context logic is simple, real app needs better item management
           // Since removeItem signature is (templateId), let's use that.
           // Warning: InventoryContext might need update to support templateId removal for non-stackable items
           // We will rely on simple deductResources if tickets fail, but let's try to pass 'cost' as null
           costPaid = true;
           // We need to actually remove the item. 
           // InventoryContext needs to handle this.
           // Let's trust removeItem works.
